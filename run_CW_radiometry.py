...
def run_CW_radiometry(input_param_path):
    ...
    laser_power                     = input_params["Laser Power [W]"]
    ...
    for range in ranges:
        ...
        # calculate laser reflection from target
        P_lrt[range] = laser_power * tgt_refl * (d**2) * (D_ap**2) * pow(tau_atmos, 2*range/1000) * tau_optics / (range**2) / (f**2) / (laser_div_angle**2) / (math.pi)
        mu_lrt[range] = P_lrt[range] * conversion_term


        # calculate laser backscatter to target
        backscatter_ranges = np.arange(r_near, range, 0.1)
        ...
        # calculate laser backscatter to background by adding to-target backscatter and past-target backscatter
        backscatter_secondary_ranges = np.arange(range, 10*range, 10)
    ...
    print("CNR", CNR[200])
    print("LRT", mu_lrt[200], "electrons")
    print("LPT", mu_lpt[200], "electrons")
...
if __name__ == '__main__':
    print("\n In run_CW_radiometry main function...")
